...
<div class="relative group">
    ...
    <button class="flex items-center hover:bg-gray-100 px-4 py-2 rounded-md cursor-pointer">
        {{$menu['label']}}
        <i class="text-xs fa fa-chevron-down ml-2 transition duration-300 ease-in-out transform group-hover:rotate-180"></i>
    </button>

    <!-- Popup (Dropdown) -->
    <div class="absolute left-0 top-full hidden w-md space-y-2 bg-white text-black p-3 rounded-md border border-gray-300 group-hover:block z-10">
        <ul class="space-y-4">
            @foreach($menu['subMenu'] as $subMenu)
            <li class="flex items-center space-x-4 hover:bg-gray-100 cursor-pointer rounded-md">
                <a href="{{$subMenu['link']}}" class="flex items-center space-x-4">
                    @if($subMenu['image'])
